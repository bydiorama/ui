...
| A fill dropped even though the token DOES exist | The same silent failure, and not only for missing tokens — a `fill` inside `write_html` can simply not land, on a perfectly valid role. `get_computed_styles` on the path returns `{}`, which is the tell: not a wrong colour, NO declaration. Re-apply with `update_styles` on the path node and read the styles back. Never conclude "the glyph is invisible so the colour must be too pale" before checking that a colour was set at all — one wasted round of re-picking tokens here was really an empty `{}` |
| Typing a path from memory because it is "just a chevron" | It is never the same path. Two glyphs went in from memory this way and both differed from the package — `ExpandSidebar` and `ChevronDownSmall` — so the sheet showed a plausible arrow that no import would ever produce. If it is worth an icon, it is worth the `node -e` extraction; if the extraction feels heavy, batch every glyph the sheet needs in ONE call |
