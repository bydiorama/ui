...
A new rule is usually wrong before the code is. `check:utilities` rule C found
exactly one thing on its first run — a FALSE POSITIVE, on Table, which had
already solved the problem properly by declaring its property through a `style`
object rather than an arbitrary class. Two spellings; the rule knew one. Budget
for that, and record which spelling you missed rather than quietly widening the
pattern.
...
| A component-level CSS custom property | It is the ONE styling surface every gate was blind to: both its spellings are arbitrary values, which `check:utilities` skips by design, so a misspelling on either side emits nothing and falls back silently. Rule C of that gate now checks the NAME in three directions — a read that resolves to nothing, a declaration nothing reads, and a story overriding a property no component has. Assert the default AND a caller's override in the browser test anyway; the gate proves the name exists, not that the value reaches layout |
| A `@ts-expect-error` on a prop belonging to ANOTHER member of the union | It will not error, and the directive reports as unused. TypeScript's excess-property check runs against the WHOLE union, so `steps` on a `form="measured"` element is "a property some member has" and is waved through. A discriminated union buys you the REQUIRED direction — the shape that would render an empty box — and nothing in the other. Test that direction, and write the finding down rather than deleting the case silently |
| A visual case sized to the capture frame | The frame is 560px wide **with 24px of padding**, so `w-[560px]` overflows its content box by 48 and the trailing edge is cropped out of the baseline forever. It is the third cause of a cropped baseline here, after the browser viewport and the iframe downscale — and the first two were both found by a person. `matrix.visual.test.tsx` now asserts `scrollWidth <= clientWidth` before every shot; size a case to the CONTENT width, or let it fill |
| Two sheets disagreeing about where a component's boundary is | The DRAWINGS settle it, not the prose. The Chat Widget — Text sheet asks for three families in one item; the Questionnaire sheet calls itself one of two. Count the parts that survive the move: a questionnaire has no container, no header and no action bar, so six of the shared component's parts mean nothing to it — §7a's Sheet-and-Drawer test, answered by what is drawn rather than by which sheet was written last. Record the disagreement in BOTH docs |
| Hand-rolling a media frame | `aspect-[1/1] overflow-clip rounded-md` is `AspectRatio`, which also sizes the picture from the outside (`[&>img]:size-full object-cover`) and keeps a well behind it — three things the call site that reinvents it has to remember, and the one that forgets ships an intrinsically-sized image in a box that crops it. A 48px one is `Thumbnail`. The same question as the `<button>` row, one primitive over: what sentence says the existing frame fails? |
| A story for a component with DISCRIMINATED-UNION props | `satisfies Meta<typeof X>` + `StoryObj<typeof meta>` makes Storybook demand the full union on every story, including the render-only ones that take no args. Use the looser `Meta<typeof X>` / `StoryObj<typeof X>`. You will not see it from the repo root: `apps/storybook` type-checks separately, so run `pnpm type-check`, never bare `tsc --noEmit` |
