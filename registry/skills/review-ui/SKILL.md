...
| `tsc` passes, `check:stories` passes, and Storybook cannot index the file | The runner | A `*/` inside BACKTICKS in a block comment terminates the comment there — everything after it is parsed as code. TypeScript was silent (the remainder happened to parse), ESLint was silent, and `check:stories` only looks at export shapes; the CSF indexer reported `SyntaxError: Missing semicolon` at a line that is visibly prose, wrapped in `Unable to index`, wrapped in `Failed to initialize projects`. It fails the WHOLE browser suite, not one file, so it reads as an environment problem. When every project fails to start, read the FIRST error, not the aggregate |
| A transition on a property nothing ever changes | Source | The motion twin of "a prop that announces and shows nothing". `transition-[opacity]` on a bar whose opacity never moves reads in review as a considered decision and animates no state that exists. Ask of every transition list: which line of this component changes that property, and under which variant? If there is no answer, delete it — `check:motion` cannot see this one, because a transition that never fires is indistinguishable from one whose trigger is elsewhere |
| A gate's first run reporting a false positive | The gate itself | `check:utilities` rule C found one thing on its first run and it was Table, which had already done the right thing — declaring its custom property through a `style` object because the value is a `calc()` an arbitrary class cannot hold. A rule that knows one of a thing's two spellings reports the file that used the other. Before treating a first-run finding as a bug, check whether the code is doing something the RULE has not met yet |
