...
└─ disclaimer?
  ...
  motion:
    "Two layers, one clock each. Interaction feedback is a transition on the frame — border-color, box-shadow and background-color at duration-fast / ease-out. The inline↔stacked flip is a measured FLIP at duration-base / ease-out via the Web Animations API: the frame's height, its border-radius and the start/actions slots are captured before the re-arrangement and animated from where they were to where they land, the frame clipped in flight. The radius rides the FLIP rather than the transition list deliberately: rounded-full computes to 999px and only the painted value is clamped to the box, so a CSS transition from it sits above the clamp for ~99% of its duration and snaps at the end — the FLIP animates measured visual radius to measured visual radius instead. It fires ONLY when measured auto layout crosses arrangements: never on mount (no entrance animation), never for an explicit layout prop (stories and visual baselines stay static), and never on ordinary line growth — the field's height is still set imperatively and deliberately NOT transitioned, because a height that eases behind the caret puts the caret outside the box while it catches up. The text row snaps on the flip for the same reason: its width change re-wraps the draft instantly either way, and glyphs gliding under a still-moving Send button read as a glitch. Reduced motion is handled twice — the WAAPI reads its duration from the computed --ui-duration-base, which the token layer collapses to 1ms, and checks prefers-reduced-motion explicitly besides.",
