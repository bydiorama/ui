...
<div className="relative overflow-clip">
  ...
  knownGaps: [
    "No scroll binding ships in the component: isVisible is caller-owned, and wiring it to overflow (scroll listeners or a ResizeObserver, per edge) is deliberate scope for a shared hook once a second consumer exists — the sheet's Gaps row (Scroll binding) records the same. The two states and the opacity transition are the component's half of that contract.",
    "No mask mode: an overlay ramp must know its ground, so over media or mixed content — where painting any colour is wrong — the right tool is mask-image on the scroll container (HeroUI's approach). Recorded on the sheet (Gaps § Implementation); it lands as an opt-in when a real consumer needs it, because a mask cannot be a drop-in span.",
