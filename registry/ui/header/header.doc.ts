...
export const headerDoc = {
  ...
  anatomy: [
    { part: "root", slot: "header", notes: "A <header>. 48px tall — PINNED (h-12), not emergent from py-sm around the tallest child: a bar whose tallest child is a 24px Header.Item used to render at 40px, silently. px-lg, bg-base — the PAGE ground, so the bar and the page it frames are one surface. Sidebar already painted bg-base; the bar was the outlier and in dark it sat below the page. Carries a transparent 1px bottom border at rest — see `affix` for why the hairline exists before it is visible. THE PAGE UNDER A HEADER MUST BE --ui-bg-base: the bar paints the page ground so the two read as one surface, and the affix Fade ramps to `base` for the same reason. That is an invariant on the CONSUMER, stated here because two shells in one app disagreed with it and nothing said so — one painted bg-surface, one a raw off-white. In light the mismatch is invisible (base and surface are 0.4% apart); in dark they are 20% apart and the ramp becomes a visible smudge under the bar. It is deliberately not a prop: the fade's ground is only correct if the bar's own fill moves with it, so a `ground` escape hatch on the ramp alone would let a consumer build a bar whose fill and ramp disagree — worse than the hardcoding it would relax." },
    ...
    { part: "fade (affix variant)", slot: "fade", notes: "Present only with affix={{ fade: true }}: a Fade (side=top, md, ground=base) hung at top-full below the bar, visible only while data-affixed. In this variant the ramp IS the separation — the shadow and the hairline colour stay off — and the page dissolves under the bar instead of sliding beneath an edge. aria-hidden, pointer-events none: it adds nothing to the banner landmark and content under it stays clickable. Drawn on the Header sheet (05 Affix · Affix + fade, both schemes) and on the Fade sheet's Pinned-header pattern." },
...
at 48px, not a mode of Sidebar — and a layout picks one (ADR 0015).
  ...
  props: {
    affix: {
      type: "boolean | { fade?: boolean }",
      default: "false",
      notes:
        "Pins the bar (sticky top-0, z on --ui-z-sticky — the chrome layer, under every floating surface) AND lets it change into its affix state once the page has moved under it. ONE prop, not two: a bar that changes appearance without being pinned describes something that did not happen. Not a controlled `isAffixed` either — that would put a scroll listener in every consumer, which is the duplication this library exists to avoid. The state is OBSERVED, never listened for: an IntersectionObserver at threshold 1 with a -1px top root margin, which answers \"am I stuck?\" off the main thread and only when the answer changes. A scroll listener would fire every frame and need a getBoundingClientRect — a forced synchronous layout — to answer the same question. No sentinel element is involved. Turning `affix` off resets the state, so a bar toggled out while stuck does not keep a floating ground with nothing to float over. `{ fade: true }` is the VARIANT (CONVENTIONS §3's boolean-or-config shape): while affixed, a Fade — side=top, md depth, ground=base — hangs at top-full below the bar and the hairline and shadow-lg stay OFF, because the ramp is the separation. The ramp's ground is FIXED at `base` and takes no override, because what scrolls under the bar is the page and the page under a Header is --ui-bg-base (see the root part). Moving it alone would decouple the ramp from the bar's own fill (Header sheet 05 Affix · Fade variant; Fade sheet, Pinned header). The bar keeps bg-affix and the backdrop blur in both modes; `{}` behaves as `true`.",
    },
  ...
  knownGaps: [
    ...
    "`affix` pins with `top-0`, fixed, and layers at `--ui-z-sticky` — the scale's chrome step, deliberately UNDER --ui-z-dropdown so a Menu opened from the bar itself (portalled to body) never slides beneath the translucent bar. A bar under another fixed element (a system banner, an editing toolbar) needs a different offset — not a prop yet; `className` is the escape hatch until a second caller needs one.",
