...
describe("render swaps the row's own tag without dropping its wiring", () => {
  test("the render element's tag and attributes carry the DOM node", async () => {
    ...
    const c = mount(
      <Sidebar label="Primary">
        <Sidebar.Item
          // A FRAGMENT, not a path. Playwright follows a real click on a real
          // <a href="/exports">, the test iframe navigates away, and the run
          // dies with "Cannot connect to the iframe" — which is a failure of
          // the fixture, not of the render slot. Same-document, same assertion.
          href="#exports"
          isCurrent
